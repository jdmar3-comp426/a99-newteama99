...
window.onload = function () {
    // Wait until everything has loaded before we run the program
    //Run startgame function when button is clicked
    id("start-btn").addEventListener("click", startGame);
    id("save-btn").addEventListener("click", saveGame);
...
function generateBoard(board) {
    ...
    for(let i=0; i<81; i++){
        ...
        if(board.charAt(i)!= "-"){
            ...
            // Ad click event listener to tile
...
//Helper functions
function id(id) {
    // returns element with the corresponding id
